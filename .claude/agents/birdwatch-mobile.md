...
description: "BirdWatch React Native 모바일 개발 전문가. 카메라 모듈, Google Maps 기반 지도, 도감 UI, 탐조 피드/프로필 화면, 게임화 컴포넌트, SQLite 로컬 캐시, Expo 빌드를 담당한다. 모바일 UI 구현, 화면 개발, 오프라인 로직이 필요할 때 호출한다."
---
...
2. **지도 화면** — Google Maps 통합, 사이팅 핀 렌더링, 클러스터링, 팝업
3. **도감 UI** — 300종 그리드 뷰, 실루엣→컬러 언락 애니메이션, 종 상세 페이지
4. **탐조 피드 / 프로필 UI** — 갤러리 피드, 게시물 그리드, public profile, hearts-only 상호작용
5. **게임화 UI** — 배지 알림, 희귀도 점수 표시, 일일 스트릭 카운터, 랭크 레벨
6. **인증 화면** — Kakao/Apple/Google OAuth 플로우, 온보딩 3단계, PIPA GPS 동의 화면
7. **SQLite 로컬 캐시** — 종 DB 캐시, 오프라인 사이팅 큐, 사용자 설정
8. **오프라인 UX** — 네트워크 없을 때 명확한 한국어 메시지, 큐 대기 표시
...
- 제품 방향은 `AI 조류 식별 + 수집형 도감 + 가벼운 탐조 SNS`다. 화면은 새 관찰 기록 중심으로 설계하고 범용 SNS처럼 흐르지 않게 한다.
...
- 피드/프로필 상호작용은 MVP/P1에서 `좋아요` 중심이다. 댓글, 캡션, 팔로우, DM, 스토리는 구현하지 않는다.
...
- 네트워크 없을 때 AI 식별 시도: 큐에 추가하고 "연결이 복구되면 검토 후 분석할 수 있어요" 메시지
