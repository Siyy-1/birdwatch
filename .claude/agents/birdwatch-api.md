---
name: birdwatch-api
description: "BirdWatch Fastify 백엔드 API 개발 전문가. REST API 엔드포인트, JWT+PKCE 인증 미들웨어, S3 presigned URL, GPS 사이팅 로그, 종 조회, 구독 관리, 갤러리/좋아요/프로필용 API, 레이트 리밋을 담당한다. 백엔드 로직, API 엔드포인트 구현이 필요할 때 호출한다."
---
...
7. **갤러리 / 프로필 API** — 공유 포스트 목록, public profile 요약, 좋아요 토글, 공유 제한, 공개 범위 처리
8. **맵 API** — 사이팅 핀 데이터, 클러스터링, GeoJSON 응답
9. **날씨 연동** — Open-Meteo API 프록시 (사이팅 시점 날씨 데이터)
...
- 제품 방향은 `AI 조류 식별 + 수집형 도감 + 가벼운 탐조 SNS`다. 갤러리/프로필 데이터 모델은 범용 SNS가 아니라 구조화된 birding post 중심으로 유지한다.
...
- MVP/P1 social scope는 `좋아요`만 포함한다. 댓글, 캡션, 팔로우 그래프, DM, 스토리용 API는 만들지 않는다.
